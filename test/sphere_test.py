""" Sphere Tests """
from math import sqrt, pi
import rt as RT

class TestSphere:
  """ features/spheres.feature """

  def test_sphere_intersect(self):
    """ A ray intersects a sphere at two points """
    r = RT.Ray(RT.Point(0, 0, -5), RT.Vector(0, 0, 1))
    s = RT.Sphere()
    xs = s.intersect(r)
    assert len(xs) == 2
    assert xs[0].t == 4.0
    assert xs[1].t == 6.0

  def test_sphere_intersect_tangent(self):
    """ A ray intersects a sphere at a tangent """
    r = RT.Ray(RT.Point(0, 1, -5), RT.Vector(0, 0, 1))
    s = RT.Sphere()
    xs = s.intersect(r)
    assert len(xs) == 2
    assert xs[0].t == 5.0
    assert xs[1].t == 5.0

  def test_sphere_intersect_misses(self):
    """ A ray misses a sphere """
    r = RT.Ray(RT.Point(0, 2, -5), RT.Vector(0, 0, 1))
    s = RT.Sphere()
    xs = s.intersect(r)
    assert len(xs) == 0

  def test_sphere_intersect_inside(self):
    """ A ray originates inside a sphere """
    r = RT.Ray(RT.Point(0, 0, 0), RT.Vector(0, 0, 1))
    s = RT.Sphere()
    xs = s.intersect(r)
    assert len(xs) == 2
    assert xs[0].t == -1.0
    assert xs[1].t == 1.0

  def test_sphere_intersect_behind(self):
    """ A sphere is behind a ray """
    r = RT.Ray(RT.Point(0, 0, 5), RT.Vector(0, 0, 1))
    s = RT.Sphere()
    xs = s.intersect(r)
    assert len(xs) == 2
    assert xs[0].t == -6.0
    assert xs[1].t == -4.0

  def test_sphere_intersect_set_object(self):
    """ Intersect sets the object on the intersection """
    r = RT.Ray(RT.Point(0, 0, -5), RT.Vector(0, 0, 1))
    s = RT.Sphere()
    xs = s.intersect(r)
    assert len(xs) == 2
    assert xs[0].object == s
    assert xs[1].object == s

  def test_sphere_default_transform(self):
    """ A sphere's default transformation """
    s = RT.Sphere()
    assert s.transform == RT.Matrix.identity()

  def test_sphere_change_transform(self):
    """ Changing a sphere's transformation """
    s = RT.Sphere()
    t = RT.Matrix.translation(2, 3, 4)
    s.transform = t
    assert s.transform == t
    s2 = RT.Sphere(t)
    assert s2.transform == t

  def test_sphere_intersect_scaled(self):
    """ Intersecting a scaled sphere with a ray """
    r = RT.Ray(RT.Point(0, 0, -5), RT.Vector(0, 0, 1))
    s = RT.Sphere(RT.Matrix.scaling(2, 2, 2))
    xs = s.intersect(r)
    assert len(xs) == 2
    assert xs[0].t == 3
    assert xs[1].t == 7

  def test_sphere_intersect_translated(self):
    """ Intersecting a translated sphere with a ray """
    r = RT.Ray(RT.Point(0, 0, -5), RT.Vector(0, 0, 1))
    s = RT.Sphere(RT.Matrix.translation(5, 0, 0))
    xs = s.intersect(r)
    assert len(xs) == 0

  def test_sphere_normal_x(self):
    """ The normal on a sphere at a point on the x axis """
    s = RT.Sphere()
    n = s.normal_at(RT.Point(1, 0, 0))
    assert n == RT.Vector(1, 0, 0)

  def test_sphere_normal_y(self):
    """ The normal on a sphere at a point on the y axis """
    s = RT.Sphere()
    n = s.normal_at(RT.Point(0, 1, 0))
    assert n == RT.Vector(0, 1, 0)

  def test_sphere_normal_z(self):
    """ The normal on a sphere at a point on the z axis """
    s = RT.Sphere()
    n = s.normal_at(RT.Point(0, 0, 1))
    assert n == RT.Vector(0, 0, 1)

  def test_sphere_normal_nonaxial(self):
    """ The normal on a sphere at a nonaxial point """
    s = RT.Sphere()
    n = s.normal_at(RT.Point(sqrt(3) / 3, sqrt(3) / 3, sqrt(3) / 3))
    assert n == RT.Vector(sqrt(3) / 3, sqrt(3) / 3, sqrt(3) / 3)

  def test_sphere_normal_normalized(self):
    """ The normal is a normalized vector """
    s = RT.Sphere()
    n = s.normal_at(RT.Point(sqrt(3) / 3, sqrt(3) / 3, sqrt(3) / 3))
    assert n == n.normalize()

  def test_sphere_normal_translated(self):
    """ Computing the normal on a translated sphere """
    s = RT.Sphere(RT.Matrix.translation(0, 1, 0))
    n = s.normal_at(RT.Point(0, 1.70711, -0.70711))
    assert n == RT.Vector(0, 0.70711, -0.70711)

  def test_sphere_normal_transformed(self):
    """ Computing the normal on a transformed sphere """
    s = RT.Sphere(RT.Matrix.scaling(1, 0.5, 1) * RT.Matrix.rotation_z(pi / 5))
    n = s.normal_at(RT.Point(0, sqrt(2) / 2, -sqrt(2) / 2))
    assert n == RT.Vector(0, 0.97014, -0.24254)

  def test_sphere_default_material(self):
    """ A sphere has a default material """
    s = RT.Sphere()
    m = s.material
    assert m == RT.Material()

  def test_sphere_assign_material(self):
    """ A sphere may be assigned a material """
    s = RT.Sphere()
    m = RT.Material()
    m.ambient = 1
    s.material = m
    assert s.material == m
